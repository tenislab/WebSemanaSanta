...
--    2. columnas-que-faltan.sql    Columnas que solo llegaban a las bases nuevas (hora de citación, cobros…)
--    3. rls-cargos.sql             Permisos por cargo de la junta
--    4. rls-endurecer.sql          EL IMPORTANTE: cierra la escritura a quien se registre
--    5. hermano-auth.sql           Acceso del hermano a su propia ficha
--    6. web-publica.sql            La web pública
--    7. mensajes-web.sql           Buzón de los formularios de la web
--    8. storage-archivo.sql        Adjuntos del archivo documental
--    9. add-provincia.sql          La provincia en la ficha de la hermandad
--   10. multi-hermandad.sql        TODAS las hermandades en una base, sin verse entre ellas
--   11. apuntes-automaticos.sql    Que los cobros lleguen solos a Tesorería
--   12. registro-actividad.sql     Quién hizo qué (RGPD, artículo 32)
--   13. remesas.sql                Que una remesa SEPA no se cobre dos veces
--   14. comunicados-segmento.sql   Guardar a quién iba dirigido un comunicado
--   15. acceso-hermano.sql         Cerrar el barrido de DNI en el acceso del hermano
--   16. area-hermano.sql           Que el área del hermano funcione de verdad
--   17. correo-hermandad.sql       Que la configuración de correo sea de la hermandad
--   18. hermano-y-gestion.sql      Ser hermano Y llevar la hermandad a la vez
--   19. permisos-por-hermandad.sql Que los permisos por cargo sean de cada hermandad
--   20. colores-hermandad.sql      Que el área del hermano lleve los colores de su hermandad
--   21. eventos-repeticion.sql     Que un culto que se repite siga repitiéndose
--   22. suscripcion.sql            Que la suscripción sea de la hermandad, no del navegador
--   23. plantillas-hermandad.sql   Que los modelos y la asistencia no se pierdan al cerrar sesión
--   24. ajustes-de-la-hermandad.sql Ajustes de cuotas y etiquetas: de la hermandad, no del navegador
--   25. hermano-con-cargo.sql      Una persona, una ficha: el cargo va en la ficha del hermano
--   26. seguridad-claves-y-registro.sql Fuera las contraseñas en claro; el registro lo escribe la base
--   27. papeletas-simbolica-y-precio.sql El precio de la papeleta y la simbólica: de la hermandad, no del navegador
--   28. buscar-hermandad-con-su-escudo.sql Que el hermano encuentre su hermandad por ciudad y la reconozca por su escudo
--   29. redes-sociales.sql         Que cada hermandad tenga sus propias redes (la clave era global)
--   30. clave-de-catalogos.sql     Que cada hermandad tenga sus propios catálogos (la clave era global)
--   31. motivo-del-rechazo.sql     Por qué se rechazó un alta, para poder decírselo a quien la pidió
--   32. imagenes.sql               El almacén de fotos: que la web no lleve las imágenes dentro
--   33. visitas-web.sql            El contador de visitas de la web, sin cookies ni Google Analytics
--   34. suscriptores-web.sql       Avisos por correo para quien sigue a la hermandad sin ser hermano
--   35. copias.sql                 Las copias de seguridad, guardadas solas cada semana
--   36. permisos-eventos-y-web.sql Los dos módulos que nunca se sembraron: «eventos» y «web»
--   37. lo-que-toca-el-hermano.sql Que el hermano no se ponga la cuota como pagada desde la consola
--   38. sin-contrasenas-en-las-solicitudes.sql Fuera la contraseña en claro que guardaba cada solicitud de alta
--   39. freno-de-los-formularios.sql Un tope a lo que cualquiera puede meter desde la web pública
--   40. cuenta-por-hermandad.sql   Ser hermano de dos hermandades: una cuenta por hermandad + DNI
--   41. solicitudes-de-papeleta.sql Que la solicitud de papeleta del hermano llegue a la hermandad
--   42. activar-la-suscripcion.sql Que el botón de activar la suscripción llegue a la base
--   43. numero-de-recibo-unico.sql Que no pueda haber dos recibos con el mismo número
--   44. borrar-una-hermandad.sql   Que una hermandad se pueda borrar (el registro lo impedía)
--   45. documentos-restringidos.sql Que el documento restringido lo sea también en la base
--   46. webhook-stripe.sql         Que la suscripción se active cuando Stripe confirma el cobro, no antes
--   47. mandatos-sepa.sql          El mandato SEPA firmado de verdad, por el propio hermano
--   48. encargos-redes.sql         Encargar un post y que se reparta solo entre la junta
--   49. tienda.sql                 La tienda: productos, ventas, stock y los asientos que generan
--   50. tienda-web.sql             La tienda en la web: reservar por internet y pagar al recoger
--   51. campanas-y-proyectos.sql   Campañas de recaudación con su barra, y proyectos a largo plazo
--   52. baja-de-hermano.sql        La baja de un hermano, entera y sin romper el escalafón
--   53. campana-con-partida.sql    Enlazar una campaña a sus partidas: la barra se llena sola
--   54. reglas-de-reparto.sql      Gastos porcentuales enlazados a una partida, para pérdidas y ganancias
--   55. pago-tarjeta.sql           Que el hermano pague su cuota o su papeleta con tarjeta
--
-- -----------------------------------------------------------------------------
