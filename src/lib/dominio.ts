 ...
 * Gobergo o la web de la hermandad que tenga puesto ese dominio. Aquí solo se
 * responde lo primero —si estamos en casa— para poder pintar la página de
 * Gobergo al momento, sin preguntar nada a la base de datos por cada visita.
 *
 * `VITE_DOMINIO_APP` es el dominio propio de Gobergo cuando lo haya. Sin
 * definirlo también funciona: entonces se consulta, se ve que ninguna
 * hermandad tiene ese dominio y se acaba enseñando la página de Gobergo
 * igualmente. Definirlo solo ahorra la consulta.
 */
export function esCasaDeGobergo(host: string, dominioApp?: string): boolean {
