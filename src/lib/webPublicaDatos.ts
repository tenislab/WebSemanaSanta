/*
 * LOS DATOS Y LOS VALORES POR DEFECTO DE LA WEB PÚBLICA. SIN NAVEGADOR.
 *
 * POR QUÉ ESTÁ PARTIDO ASÍ, QUE ES LO ÚNICO RARO DE ESTE FICHERO:
 *
 * `webPublica.ts` importa React, `./supabase` y `localStorage`. Cualquier
 * cosa que se importe de ahí se lleva las tres detrás, y una función de
 * servidor que las arrastre PASA DE 10 KB A 980 KB y revienta al arrancar
 * (`import.meta.env` no existe en Node). Ya pasó una vez y tiró la puerta
 * principal de gobergo.com; lo cuenta `webPublicaPuro.ts` con detalle.
 *
 * Y `api/w.ts` necesita `conDefectos`: sin él le pasa al generador del SEO el
 * JSON crudo de la base, y una web guardada antes de que un campo existiera
 * lo hace fallar en silencio —la vista previa de WhatsApp y Google de esa
 * hermandad vuelve a decir «Gobergo — Software para gestionar tu hermandad»
 * sin que nadie se entere—. Está reproducido en `webamedias.prueba.mjs`.
 *
 * Así que aquí vive todo lo que NO toca el navegador: los tipos, los valores
 * iniciales, los guiones de ejemplo y `conDefectos`. `webPublica.ts` lo
 * reexporta entero, así que el resto de la aplicación lo sigue pidiendo allí
 * como siempre y nada cambia de sitio para quien lo usa.
 *
 * REGLA: este fichero no importa nada en tiempo de ejecución. Ni React, ni
 * `./supabase`, ni `import.meta.env`, ni `localStorage`. Si algún día hace
 * falta algo de eso, va en `webPublica.ts`, no aquí.
 */
...
/*
 * ============================================================================
 * LAS OTRAS LISTAS, TAMBIÉN CON SU ELEMENTO VACÍO
 * ============================================================================
 *
 * Titulares, cultos y carteles ya se completaban en `conDefectos`, cada uno
 * porque alguien se estrelló con ellos. Noticias, páginas, junta, horarios y
 * redes no, y estaban esperando: con un elemento guardado por una versión
 * anterior —al que le falta un campo que entonces no existía— LA WEB PÚBLICA
 * SE CAE AL PINTAR y el visitante ve «Algo se ha roto al abrir esta pantalla».
 *
 * Medido, no supuesto. Sembrando `paginas: [{ id: 'x1' }]` o
 * `junta: [{ id: 'x1' }]` y abriendo /w/demo:
 *
 *   · páginas → «Cannot read properties of undefined (reading 'filter')»
 *     (`SitioContenido.tsx`, el cuerpo de la página hace `parrafos.filter`).
 *   · junta   → «Cannot read properties of undefined (reading 'trim')»
 *     (`SitioContenido.tsx:220`, `web.junta.some((m) => m.cargo.trim())`).
 *
 * Y no es un caso raro: es lo NORMAL cada vez que se añade un campo. La web
 * vive en una columna `jsonb` y en el navegador; lo que llega es lo que se
 * guardó el día que se guardó.
 */
export const NOTICIA_VACIA: Omit<Noticia, 'id'> = {
  titulo: '', fecha: '', resumen: '', fotoDataUrl: null, altFoto: '', publicada: false, parrafos: [],
}
export const PAGINA_VACIA: Omit<PaginaWeb, 'id'> = {
  icono: '', antetitulo: '', titulo: '', entradilla: '', parrafos: [], fotos: [], enMenu: true,
}
export const MIEMBRO_JUNTA_VACIO: Omit<MiembroJunta, 'id'> = { cargo: '', nombre: '' }
export const FRANJA_HORARIO_VACIA: Omit<FranjaHorario, 'id'> = { dias: '', horas: '', nota: '' }
export const RED_VACIA: Omit<RedWeb, 'id'> = { tipo: 'Facebook', url: '' }
...
export function conDefectos(guardado: Partial<WebPublica> | null): WebPublica {
  ...
  return {
    ...
    /*
     * Y LAS CINCO QUE FALTABAN, por lo mismo y con la misma forma (las otras
     * tres van más abajo, donde ya estaban declaradas).
     *
     * Un elemento al que le falta un campo que no existía cuando se guardó
     * TIRA LA WEB PÚBLICA ENTERA al pintarla: está reproducido en
     * `pruebas/webamedias.prueba.mjs`. Con `lista()` además se sobrevive a un
     * `null` o a un objeto donde debía haber una lista, que es lo que llega de
     * una copia antigua a medio migrar.
     */
    junta: lista(guardado.junta).map((m) => ({ ...MIEMBRO_JUNTA_VACIO, ...m })),
    horarios: lista(guardado.horarios).map((h) => ({ ...FRANJA_HORARIO_VACIA, ...h })),
    caridad: {
      ...CARIDAD_VACIA,
      ...(guardado.caridad ?? {}),
    ...
    albumes,
    noticias: lista(guardado.noticias).map((n) => ({ ...NOTICIA_VACIA, ...n })),
    paginas: lista(guardado.paginas).map((x) => ({ ...PAGINA_VACIA, ...x })),
    ...
    redes: lista(guardado.redes).map((r) => ({ ...RED_VACIA, ...r })),
  }
}
